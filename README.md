...
KAM 把 AI 写代码，变成 AI 持续做工程工作。

它不是另一个聊天窗口，也不是只会跑一次 prompt 的 agent wrapper。
KAM 是一套 `local-first` 的 AI 工程控制面：把 GitHub issue、PR 评论和手工任务接进来，放进统一任务池，持续推进，并且全程可追踪、可打断、可接管。
...
AI 写代码已经不稀缺了。真正稀缺的是，怎么把工程工作接住、做下去、别失控。

真实的软件工程 agent 工作，通常卡在这些地方：
...
KAM 的产品设计就是围绕这些问题做的：让软件工程工作像一个可操作、可监督、可持续推进的任务系统一样运行，而不是像一次次临时对话。

## 它和 Cursor / Claude Code / Devin 有什么不同

这些产品主要解决的是“agent 怎么写代码”。

KAM 解决的是另一层问题：

`agent 写出来的工程工作，怎么持续运转、被接住、被监督、被接管。`

所以 KAM 的差异不在模型，而在控制层：

- `Task-native`
  工作先进入任务池，再开始执行，不再靠一段段临时对话撑着
- `With receipts`
  每一轮都有 runs、artifacts、compare 和 follow-up，不是黑盒
- `Operator-native`
  可以继续、重试、采纳、打断、重启，不会失去控制权
- `Local-first`
  代码、执行和恢复语义都在你自己的环境里，不被纯云端工作流锁死

## 适合谁

- 想在本机持续驱动 agent 干活的个人开发者
- 想把 GitHub issue / PR 评论接进统一任务池的人
- 想让工程工作自动继续推进，但又必须保留人工接管能力的小团队
- 想做 `KAM builds KAM` 这种高频 dogfood 的仓库维护者
...
- KAM 可以围绕当前 task family 自动决定下一步动作，而不是只跑一轮
...
这也是 KAM 当前最值得打透的 wedge：

`GitHub issue / PR comment -> task pool -> run -> artifacts -> follow-up -> continue`

## 第一次打开先看什么

现在首页默认先讲三件事：

- `Task-native`：工作先进入任务池，再开始执行
- `With receipts`：每一轮都有 runs、artifacts 和 compare
- `Operator-native`：系统可以继续推进，但人工随时能接管

如果你播种 demo harness 数据，默认看到的不是一张随便写的 demo task，而是一条 GitHub issue 场景：

`GitHub issue -> task -> snapshot -> run -> artifacts -> compare -> continue`

最推荐的体验顺序是：

1. 先看首页的差异点证据区和默认演示链路
2. 再打开当前 task，看 refs、snapshot、runs、artifacts 和 compare
3. 最后决定是点推荐动作继续推进，还是手工接管
...
这里的核心不是“AI 替你点一个按钮”，而是“整个工作过程有对象、有状态、有证据、有恢复语义，而且你始终保留控制权”。
...
如果你只想要一个聊天式 coding assistant，KAM 不适合你。
如果你想让 AI 工程任务持续推进，而且必须可控，KAM 就是为这个场景做的。
...
这条命令现在会默认播种一条 GitHub issue 驱动的演示链路，方便你直接看到：

- issue 怎么进入任务池
- run 怎么留下 artifacts
- compare 和 follow-up 怎么把下一步接起来
