...
- 连续推进：KAM 不只会拆任务、接下一张，还能基于当前 task family 自动判断 `adopt / retry / plan_and_dispatch / stop`，并在显式开启后持续自动托管
...
### 4.8 Task Self-Continue

- KAM 可以围绕当前 task family 自动继续推进，而不只是等用户手动点 `retry / adopt / dispatch`
- 当前最小动作集合是：
  - `adopt`
  - `retry`
  - `plan_and_dispatch`
  - `stop`
- 自动继续推进必须优先复用现有 `runs / planner / dispatcher / adopt` 能力，而不是额外引入平行 runtime
- 如果当前作用域里仍有 `pending / running` run，continue 必须显式停下而不是重复开跑

### 4.9 Task Family Auto-Drive

- KAM 可以对当前 task family 显式开启无人值守
- 开启后，family 内任一 run 完成都会触发下一轮 `continue_task()`，直到得到 `stop`
- 自动托管当前只作用于选中的 task family，不自动扩展成全局任务池接活
- 自动托管状态写回 root task metadata，并在工作台可见
- 必须支持显式停止自动托管，且停止后不应中断已经在跑的 run，只停止后续续跑
...
- 能围绕当前 task family 自动继续推进，并返回明确动作：`adopt / retry / plan_and_dispatch / stop`
- 能对当前 task family 开启无人值守，并在 run 完成后自动续跑到 `stop`
